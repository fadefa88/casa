...
The process checks the public IP from the mini-PC, classifies the active WAN,
records failover start/end events, persists a JSON status file and exposes a
small local HTTP API. It uses only the Python standard library.
"""
...
class Monitor:
    ...
    def _parse_networks(values: Iterable[str]) -> tuple[ipaddress._BaseNetwork, ...]:
        ...
        for value in values:
            try:
                suffix = value if "/" in value else f"{value}/32"
    ...
    def _load_status(self) -> Status:
        try:
            ...
            allowed = {field.name for field in Status.__dataclass_fields__.values()}
    ...
    def _matches(self, ip_text: str, networks: tuple[ipaddress._BaseNetwork, ...]) -> bool:
        ip_value = ipaddress.ip_address(ip_text)
        return any(ip_value in network for network in networks)

    def classify(self, ip_text: str) -> str:
        ...
        if self.primary_rules and self._matches(ip_text, self.primary_rules):
            return "primary"
        if self.baseline_ip:
            return "primary" if ip_text == self.baseline_ip else "backup"
    ...
    def fetch_public_ip(self) -> tuple[str, str]:
        ...
        for provider in self.config.providers:
            request = Request(provider, headers={"User-Agent": "CasaNetworkMonitor/1.0"})
            try:
    ...
    def check_once(self) -> None:
        ...
        try:
            ...
            new_link = self.classify(public_ip)
            with STATUS_LOCK:
            ...
            LOG.info("Public IP %s classified as %s", public_ip, new_link)
...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if self.path.rstrip("/") == "/healthz":
            ...
            self._send_json(status, {"healthy": snapshot.get("healthy"), "checked_at": snapshot.get("checked_at")})
            return
