(() => {
  ...
  const APPLIANCES = [
    { key: 'washerPower', label: 'Lavatrice', aliases: ['lavatrice', 'washing machine', 'washer'] },
    { key: 'dryerPower', label: 'Asciugatrice', aliases: ['asciugatrice', 'tumble dryer', 'dryer'] },
    { key: 'ovenPower', label: 'Forno', aliases: ['forno', 'oven'] },
    { key: 'fridgePower', label: 'Frigorifero', aliases: ['frigorifero', 'frigo congelatore', 'frigo', 'congelatore', 'fridge freezer', 'fridge', 'freezer'] },
    { key: 'dishwasherPower', label: 'Lavastoviglie', aliases: ['lavastoviglie', 'dishwasher', 'lavapiatti'] },
  ];
  ...
  const TECH_GROUPS = [
    {
      ...
      icon: 'fa-tv',
      devices: [
        { key: 'tvPower', label: 'TV', aliases: ['tv', 'televisore', 'smart tv', 'samsung tv'] },
        { key: 'shieldPower', label: 'Nvidia Shield', aliases: ['nvidia shield', 'shield tv', 'shield'] },
        { key: 'mediaPcPower', label: 'Mini PC', aliases: ['mini pc', 'minipc', 'media mini pc'] },
        { key: 'hddPower', label: 'HDD', aliases: ['hdd', 'hard disk', 'disco esterno'] },
      ],
    },
    {
      id: 'studio-gaming',
      ...
      devices: [
        ...
        { key: 'monitorPower', label: 'Monitor', aliases: ['monitor', 'monitor studio', 'office monitor'] },
        { key: 'ps5Power', label: 'PS5', aliases: ['ps 5', 'ps5', 'playstation 5', 'playstation5'] },
        { key: 'dockPower', label: 'Splitter', aliases: ['splitter', 'dock studio', 'office dock', 'dock'] },
      ],
    },
  ];
  ...
  function entityParts(entity) {
    const entityName = normalize(String(entity?.entity_id || '').replace(/^sensor\./, ''));
  ...
  function aliasScore(value, alias) {
    ...
    if (normalizedValue === normalizedAlias) return 100;
    if (compactValue === compactAlias) return 96;

    const suffixes = ['power', 'potenza', 'consumo', 'presa', 'plug', 'switch'];
    if (suffixes.some((suffix) => normalizedValue === `${normalizedAlias} ${suffix}`)) return 92;
    if (suffixes.some((suffix) => compactValue === `${compactAlias}${suffix}`)) return 90;

    const boundary = new RegExp(`(^| )${normalizedAlias.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}( |$)`);
    if (boundary.test(normalizedValue)) return normalizedAlias.length <= 2 ? 72 : 82;
    if (compactAlias.length >= 3 && compactValue.includes(compactAlias)) return 68;
    return 0;
  }
  ...
  function resolvePower(rule) {
    ...
    for (const entity of map.values()) {
      ...
      const excluded = (rule.exclude || []).some((term) => {
        ...
        return parts.friendly.includes(normalized) || parts.entityName.includes(normalized);
      });
      ...
      for (const alias of rule.aliases || []) {
        score = Math.max(
          score,
          aliasScore(parts.friendly, alias),
          aliasScore(parts.entityName, alias) - 4
        );
      }
  ...
  function ensureStyles() {
    if (document.getElementById('energy-devices-v60-styles')) return;
    document.getElementById('energy-devices-v59-styles')?.remove();
    const style = document.createElement('style');
    style.id = 'energy-devices-v60-styles';
    style.textContent = `
  ...
  function patchAppliances() {
    ...
    APPLIANCES.forEach((rule) => {
      ...
      setText(metricRow(card, rule.label)?.querySelector('strong'), formatPower(value));
  ...
  function patchTechnology() {
    ...
    TECH_GROUPS.forEach((group) => {
      ...
      group.devices.forEach((device) => {
        ...
        setText(card.querySelector(`[data-tech-device="${device.key}"] strong`), formatPower(value));
        if (Number.isFinite(value)) {
          groupTotal += value;
          groupFound += 1;
          grandTotal += value;
      ...
      setText(card.querySelector(`[data-tech-total="${group.id}"]`), groupFound ? formatPower(groupTotal) : NULL_TEXT);
    });
